...
#生成用于存放pyd文件的文件夹
if os.path.exists(folder_for_build): shutil.rmtree(folder_for_build)
...
#把pyd文件移动文件夹中
for path in glob(r'*.pyd'): shutil.move(path,folder_for_build)

#移除旧的gamemode文件夹
if os.path.exists('gamemode'): shutil.rmtree('gamemode')
#生成游戏本体源代码的c和pyd文件
